...
- **GGUF-COMPLIANT PASS NOW:** `image/qwen_image.py` through `qwen_image_2512_t2i_gguf`, `image/qwen_image_edit.py` through `qwen_image_edit_2511_multi_gguf`, `image/flux2_dev.py` through `flux2_dev_gguf_quality`, `video/wan_ti2v_5b.py` through `wan22_ti2v_5b_720p24_gguf`, and `text/moviigen_rewriter.py` through `llamacpp_prompt_rewriter` currently have PASS records whose primary inference backbone is GGUF. Qwen Image 2512, Qwen Image Edit 2511, FLUX.2 Dev, and Wan use GGUF diffusion/video backbones with Safetensors text encoders where the text-encoder rule above permits them; MoviiGen uses the local Qwen2.5 Q5 GGUF served by llama.cpp.

...
- **GGUF WORK STILL NOT DONE:** `image/qwen_image_edit.py` (`qwen_image_edit_2511_multi_gguf`) is a registered GGUF target but still blocked for lack of one-reference and three-reference real certification. Original `video/wan_t2v.py` and `video/wan_i2v.py` still need Wan A14B high/low-noise GGUF Comfy workflows and real T2V/I2V artifacts. The LTX rows have Q5 GGUF assets in the owned cache, but the production plugin paths still need Comfy workflow ports and real artifacts before any GGUF completion claim.
...
- **CERTIFICATION SUMMARY:** `python -m slopperly.audit.dropdown_certification --profile smoke_16gb --report-only` now reports `PASS qwen_image_2512_t2i_gguf`, `PASS flux2_dev_gguf_quality`, `PASS flux2_klein_4b_t2i_edit`, `PASS flux2_klein_9b_t2i_edit`, `PASS flux2_klein_9b_schematic_lora`, `PASS wan22_ti2v_5b_720p24_gguf`, `PASS vllm_whisper_large_v3_turbo_stt`, `PASS vllm_video_caption_vlm`, `PASS florence2_caption_ocr`, `PASS birefnet_rmbg`, `PASS anima_t2i_i2i`, `PASS ernie_image_t2i`, `PASS ernie_image_turbo_t2i`, `PASS krea2_base_t2i`, `PASS krea2_turbo_t2i`, `PASS audio_stem_split_demucs`, `PASS mmaudio_video_to_audio`, `PASS stable_audio_3_medium_base`, `PASS ace_step_15_music`, `PASS foundation1_music_loop`, `PASS chatterbox_tts_vc_comfy`, `PASS chatterbox_turbo_tts_comfy`, `PASS chatterbox_multilingual_tts_comfy`, `PASS llamacpp_prompt_rewriter`, `PASS omnivoice_vllm_omni`, `PASS moss_tts_nano_vllm_omni`, and 14 blocked entries still requiring the next model install/runtime/artifact test.
...
| `image/_krea2_base.py` Krea 2 Base | DONE ON SPEC for `smoke_16gb`: Comfy wrapper/workflow pack uses owned Comfy with local Krea RAW diffusion, Qwen3-VL text encoder, Qwen VAE, local `TextGenerate`, and `Krea2BasePlugin.generate()` produced a validated PNG artifact. | No remaining production blocker for the certified profile; broader devices and any specific custom LoRA file still need their own artifact certification before being claimed. | Prompt, negative prompt, resolution, frames/batch, steps, guidance/cfg, seed, sampler, scheduler, local model filenames, and selected LoRA filenames/weights are patched into the owned Comfy workflow. Dynamic LoRA mapping inserts `LoraLoaderModelOnly` nodes before `KSampler`. | PASS: `.slopperly/gpu-artifacts/smoke_16gb/krea2_base_t2i/krea2_base_t2i.png`, RGB PNG, 1024x1024, through `Krea2BasePlugin.generate()`. |
| `image/krea2_turbo.py` Krea 2 Turbo | DONE ON SPEC for `smoke_16gb`: Comfy wrapper/workflow pack uses owned Comfy with local Krea Turbo diffusion, Qwen3-VL text encoder, Qwen VAE, local `TextGenerate`, and `Krea2TurboPlugin.generate()` produced a validated PNG artifact with the 8-step Turbo profile. | No remaining production blocker for the certified Turbo profile; the existing negative prompt field is preserved and surfaced in `usage_note` because the official Turbo Comfy graph uses `ConditioningZeroOut` instead of a negative text branch. Broader devices and specific custom LoRA files still need their own certification before being claimed. | Prompt, resolution, frames/batch, steps default 8, guidance/cfg default 1.0, seed, sampler, scheduler, local model filenames, and selected LoRA filenames/weights are patched into the owned Comfy workflow. Dynamic LoRA mapping inserts `LoraLoaderModelOnly` nodes before `KSampler`; Turbo negative prompt remains an explicit official-graph limitation. | PASS: `.slopperly/gpu-artifacts/smoke_16gb/krea2_turbo_t2i/krea2_turbo_t2i.png`, RGB PNG, 1024x1024, through `Krea2TurboPlugin.generate()`. |
| `image/anima.py` Anima | DONE ON SPEC for `smoke_16gb`: Comfy wrapper and T2I/I2I workflow packs use owned Comfy with local Anima diffusion, Qwen 0.6B text encoder, Qwen VAE, and `AnimaPlugin.generate()` produced validated local T2I and I2I PNG artifacts. | No remaining production blocker for the certified profile; broader devices and any specific custom LoRA file still need their own artifact certification before being claimed. | Prompt, negative prompt, selected image strip upload for I2I, resolution, frames/batch, steps, guidance/cfg, strength-to-denoise, seed, sampler, scheduler, local model filenames, and selected LoRA filenames/weights are patched into the owned Comfy workflows. Dynamic LoRA mapping inserts `LoraLoaderModelOnly` nodes before the T2I or I2I `KSampler`. | PASS: `.slopperly/certification/smoke_16gb/anima_t2i_i2i.json`, manifest plus `anima_t2i.png` and `anima_i2i.png`, both RGB PNG 1024x1024 through `AnimaPlugin.generate()`. |
...
| `image/ernie.py` ERNIE Image | DONE ON SPEC for `smoke_16gb`: Comfy wrapper and `ernie_image_t2i` workflow pack use owned Comfy with local ERNIE diffusion, Ministral text encoder, ERNIE prompt enhancer, FLUX.2 VAE, and `ErniePlugin.generate()` produced a validated PNG artifact. | No remaining production blocker for the certified base profile; broader devices and future ERNIE profiles still need their own artifact certification before exposure. | Prompt, negative prompt, resolution, frames/batch, steps, guidance/cfg, seed, sampler, scheduler, local model filenames, and local `TextGenerate` prompt-enhancer settings are patched into the owned Comfy workflow. | PASS: `.slopperly/certification/smoke_16gb/ernie_image_t2i.json`, RGB PNG 1024x1024 through `ErniePlugin.generate()`. |
| `image/ernie_turbo.py` ERNIE Turbo | DONE ON SPEC for `smoke_16gb`: Comfy wrapper and `ernie_image_turbo_t2i` workflow pack use owned Comfy with local ERNIE Turbo diffusion, Ministral text encoder, ERNIE prompt enhancer, FLUX.2 VAE, and `ErnieTurboPlugin.generate()` produced a validated PNG artifact with the 8-step Turbo profile. | No remaining production blocker for the certified Turbo profile; the existing negative prompt field is preserved and surfaced in `usage_note` because the official Turbo Comfy graph uses `ConditioningZeroOut` instead of a negative text branch. Broader devices still need their own artifact certification before exposure. | Prompt, resolution, frames/batch, steps default 8, guidance/cfg default 1.0, seed, sampler, scheduler, local model filenames, and local `TextGenerate` prompt-enhancer settings are patched into the owned Comfy workflow; Turbo negative prompt remains an explicit official-graph limitation. | PASS: `.slopperly/certification/smoke_16gb/ernie_image_turbo_t2i.json`, RGB PNG 1024x1024 through `ErnieTurboPlugin.generate()`. |
...
| `image/flux2_klein_4b.py` FLUX.2 Klein 4B | DONE ON SPEC for the certified `smoke_16gb` 16GB-default profile: native FP8 Comfy workflow packs use owned Comfy with `flux-2-klein-4b-fp8.safetensors`, `qwen_3_4b.safetensors`, FLUX.2 VAE, and `Flux2Klein4BPlugin.generate()` produced validated T2I and two-reference edit PNG artifacts. | No remaining production blocker for the certified T2I/edit profile; the official `ReferenceLatent` edit graph has no denoise/strength or mask input, so image strength and masked inpaint remain documented modes until a separate local mask/strength workflow passes artifact certification. Specific user-selected LoRA files still need their own artifact proof before being claimed as compatible. | Prompt, image strip/edit mode, up to three compressed reference inputs, resolution, frames/batch, steps, guidance/cfg, seed, sampler, local model filenames, selected LoRA filenames/weights, and optional reference branch pruning are patched into the owned Comfy workflows. Dynamic LoRA mapping inserts `LoraLoaderModelOnly` nodes before `CFGGuider.model`. | PASS: `.slopperly/certification/smoke_16gb/flux2_klein_4b_t2i_edit.json`, manifest plus `flux2_klein_4b_t2i.png` and `flux2_klein_4b_edit.png`, both RGB PNG 1024x1024 through `Flux2Klein4BPlugin.generate()`. |
| `image/flux2_klein_9b.py` FLUX.2 Klein 9B | DONE ON SPEC for the certified `smoke_16gb` device profile: native FP8 Comfy workflow packs use owned Comfy with `flux-2-klein-9b-fp8.safetensors`, `qwen_3_8b_fp8mixed.safetensors`, `full_encoder_small_decoder.safetensors`, and `Flux2Klein9BPlugin.generate()` produced validated T2I and two-reference edit PNG artifacts. | No remaining production blocker for the certified T2I/edit profile; the official `ReferenceLatent` edit graph has no denoise/strength or mask input, so image strength and masked inpaint remain documented modes until a separate local mask/strength workflow passes artifact certification. Specific user-selected LoRA files still need their own artifact proof before being claimed as compatible. | Prompt, image strip/edit mode, up to three compressed reference inputs, resolution, frames/batch, steps, guidance/cfg, seed, sampler, local model filenames, selected LoRA filenames/weights, and optional reference branch pruning are patched into the owned Comfy workflows. Dynamic LoRA mapping inserts `LoraLoaderModelOnly` nodes before `CFGGuider.model`. | PASS: `.slopperly/certification/smoke_16gb/flux2_klein_9b_t2i_edit.json`, manifest plus `flux2_klein_9b_t2i.png` (`465484` bytes) and `flux2_klein_9b_edit.png` (`968235` bytes), both RGB PNG 1024x1024 through `Flux2Klein9BPlugin.generate()`. |
| `image/flux2_klein_9b_schematic.py` schematic LoRA | DONE ON SPEC for the certified `smoke_16gb` profile: native FLUX.2 Klein 9B base plus schematic LoRA Comfy workflow pack uses owned Comfy with the base 9B FP8 diffusion, full Qwen 3 8B text encoder, FLUX.2 VAE, and all six schematic LoRA files; `Flux2Klein9BSchematicPlugin.generate()` produced validated local PNG artifacts for every exposed schematic mode. | No remaining production blocker for the certified six-mode profile; the plugin has no negative-prompt UI section, so the upstream fixed negative prompt remains an internal workflow constant rather than a missing user control. Broader devices still need their own certification before exposure there. | Required image strip upload, prompt, schematic mode selector to exact LoRA filename, segmentation target for BINARY_SEG/AMODAL_SEG prompts, frames/batch, steps, guidance/cfg, seed, base model, text encoder, VAE, and per-run output path are patched into the owned Comfy workflow. | PASS: `.slopperly/certification/smoke_16gb/flux2_klein_9b_schematic_lora.json`, manifest plus six RGB PNG 1024x1024 artifacts for DEPTH, NORMAL, BODY_POSE, FULL_POSE, BINARY_SEG, and AMODAL_SEG through `Flux2Klein9BSchematicPlugin.generate()`. |

...
- **DONE ON SPEC:** `audio/_stable_audio_3.py` Stable Audio 3, `audio/ace_step.py` ACE-Step, `audio/foundation_music.py` Foundation-1, `audio/mmaudio.py` MMAudio, `audio/chatterbox.py` Chatterbox, `audio/chatterbox_turbo.py` Chatterbox Turbo, `audio/chatterbox_multilingual.py` Chatterbox Multilingual, `audio/omnivoice.py` OmniVoice, `audio/moss_tts.py` MOSS-TTS, `text/faster_whisper_transcribe.py` STT, `text/marlin_video_captions.py` Marlin Video Captions, `text/moviigen_rewriter.py` prompt rewriter, `text/florence2.py` Florence2, `image/anima.py` Anima, `image/birefnet.py` BiRefNet, `image/ernie.py` ERNIE Image, `image/ernie_turbo.py` ERNIE Turbo, `image/flux2_dev.py` FLUX.2 Dev quality profile, `image/flux2_klein_4b.py` FLUX.2 Klein 4B 16GB default profile, `image/flux2_klein_9b.py` FLUX.2 Klein 9B certified device profile, `image/flux2_klein_9b_schematic.py` FLUX.2 Klein 9B Schematic LoRA profile, `image/_krea2_base.py` Krea 2 Base, `image/krea2_turbo.py` Krea 2 Turbo, and the certified four-stem path of `audio/stem_split.py` Stem Splitter from baseline commit `50cf377bf11349685010acb726a5e2b2e3cb9962` now have real local `ModelPlugin.generate()` PASS artifacts on the RTX 4090 profile. The Comfy-backed entries run through owned Slopperly ComfyUI; OmniVoice and MOSS-TTS run through the owned local vLLM-Omni runtime; STT and Marlin run through the owned local vLLM runtime; MoviiGen runs through the owned local llama.cpp runtime. Stem Splitter's old six-stem option, Chatterbox's true two-audio cross-speaker VC, Krea 2 Turbo's official no-negative-branch graph semantics, ERNIE Turbo's official no-negative-branch graph semantics, FLUX.2 Dev's first-three-reference certified graph limit, and FLUX.2 Klein 4B/9B official no-strength/no-mask reference-edit graph limitation remain documented modes until separate UI/workflow certification exists.

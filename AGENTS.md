...
- **GGUF-COMPLIANT PASS NOW:** `image/qwen_image.py` through `qwen_image_2512_t2i_gguf`, `image/qwen_image_edit.py` through `qwen_image_edit_2511_multi_gguf`, `image/flux2_dev.py` through `flux2_dev_gguf_quality`, `video/wan_ti2v_5b.py` through `wan22_ti2v_5b_720p24_gguf`, `video/ltx2.py` through `ltx23_i2v_q5_gguf`, and `text/moviigen_rewriter.py` through `llamacpp_prompt_rewriter` currently have PASS records whose primary inference backbone is GGUF. Qwen Image 2512, Qwen Image Edit 2511, FLUX.2 Dev, Wan, and LTX use GGUF diffusion/video backbones with Safetensors text encoders where the text-encoder rule above permits them; MoviiGen uses the local Qwen2.5 Q5 GGUF served by llama.cpp.
- **LOCAL PASS BUT NOT GGUF-COMPLIANT IMAGE COMPLETION:** `image/anima.py`, `image/ernie.py`, `image/ernie_turbo.py`, `image/flux2_klein_4b.py`, `image/flux2_klein_9b.py`, `image/flux2_klein_9b_schematic.py`, `image/_krea2_base.py`, and `image/krea2_turbo.py` have real local RTX artifacts, but their committed certified profiles use Safetensors/FP8/native Comfy backbones. They are artifact evidence only and must not be used as proof that the GGUF migration requirement is complete. As of the 2026-06-28 GGUF dropdown gate correction, `dropdown_certification` blocks these entries until a GGUF backbone is installed, wired, and rerun through the plugin-path artifact test.
- **DONE ON SPEC FOR NON-GGUF-RUNTIME ROWS:** `audio/_stable_audio_3.py` Stable Audio 3, `audio/ace_step.py` ACE-Step, `audio/foundation_music.py` Foundation-1, `audio/mmaudio.py` MMAudio, `audio/chatterbox.py` Chatterbox, `audio/chatterbox_turbo.py` Chatterbox Turbo, `audio/chatterbox_multilingual.py` Chatterbox Multilingual, `audio/omnivoice.py` OmniVoice, `audio/moss_tts.py` MOSS-TTS, `text/faster_whisper_transcribe.py` STT, `text/marlin_video_captions.py` Marlin Video Captions, `text/florence2.py` Florence2, `image/birefnet.py` BiRefNet, and the certified four-stem path of `audio/stem_split.py` Stem Splitter from baseline commit `50cf377bf11349685010acb726a5e2b2e3cb9962` have real local `ModelPlugin.generate()` PASS artifacts on the RTX 4090 profile through their required local runtimes. Stem Splitter's old six-stem option and Chatterbox's true two-audio cross-speaker VC remain documented modes until separate UI/workflow certification exists.
- **GGUF WORK STILL NOT DONE:** Original `video/wan_t2v.py` and `video/wan_i2v.py` still need Wan A14B high/low-noise GGUF Comfy workflows and real T2V/I2V artifacts. `video/ltx2.py` is certified only for the existing short I2V Q5 path through `ltx23_i2v_q5_gguf`; LTX T2V, 1080p, extend, lipsync, multi-anchor, and IC-LoRA rows still need Comfy workflow ports and real artifacts before any GGUF completion claim.
- **CERTIFIED NEW LOCAL DEFAULT:** new `video/wan_ti2v_5b.py` now has fresh direct T2V and direct I2V PASS records through `WanTI2V5BPlugin.generate()` against owned Slopperly ComfyUI on the RTX 4090 profile. This is not original Palladium parity; original Wan A14B, LTX staged workflows, and other baseline functions still need their own migrations and real artifacts.
- **CERTIFICATION SUMMARY:** `python -m slopperly.audit.dropdown_certification --profile smoke_16gb --report-only` now reports GGUF-compliant/image-safe `PASS qwen_image_2512_t2i_gguf`, `PASS qwen_image_edit_2511_multi_gguf`, `PASS flux2_dev_gguf_quality`, `PASS wan22_ti2v_5b_720p24_gguf`, `PASS ltx23_i2v_q5_gguf`, plus local-runtime `PASS vllm_whisper_large_v3_turbo_stt`, `PASS vllm_video_caption_vlm`, `PASS florence2_caption_ocr`, `PASS birefnet_rmbg`, `PASS audio_stem_split_demucs`, `PASS mmaudio_video_to_audio`, `PASS stable_audio_3_medium_base`, `PASS ace_step_15_music`, `PASS foundation1_music_loop`, `PASS chatterbox_tts_vc_comfy`, `PASS chatterbox_turbo_tts_comfy`, `PASS chatterbox_multilingual_tts_comfy`, `PASS llamacpp_prompt_rewriter`, `PASS omnivoice_vllm_omni`, and `PASS moss_tts_nano_vllm_omni`. It now blocks the native/FP8 rows `flux2_klein_4b_t2i_edit`, `flux2_klein_9b_t2i_edit`, `flux2_klein_9b_schematic_lora`, `anima_t2i_i2i`, `ernie_image_t2i`, `ernie_image_turbo_t2i`, `krea2_base_t2i`, and `krea2_turbo_t2i` with explicit GGUF-next-action diagnostics. Certification records: 20 passed, 21 blocked.
...
- Existing LTX workflow source to port before making a new graph: `/home/user/Documents/Comfy/workflows/*.json`, including many LTX 2.3 Q5/NVFP4/FP8 API workflows and known-good I2V variants. The short I2V Q5 variant is now ported/certified as `ltx23_i2v_q5_gguf`; the extend/lipsync/multi/IC variants still need their own ports and artifacts.
...
- Krea 2 files are now installed in the owned Comfy model cache: `models/diffusion_models/krea2_raw_fp8_scaled.safetensors` (`13G`), `models/diffusion_models/krea2_turbo_fp8_scaled.safetensors` (`13G`), `models/text_encoders/qwen3vl_4b_fp8_scaled.safetensors` (`4.9G`), and `models/vae/qwen_image_vae.safetensors` (`243M`).
- Owned Slopperly Comfy was live-tested on `127.0.0.1:8190` for Krea 2 Base and Krea 2 Turbo with `UNETLoader`, `CLIPLoader(type=krea2)`, `VAELoader`, `TextGenerate`, `CLIPTextEncode`, `ConditioningZeroOut`, `EmptyLatentImage`, `KSampler`, `VAEDecode`, `SaveImage`, and `LoraLoaderModelOnly` available in `/object_info`. The first real attempt exposed a codable Comfy v3 dynamic-combo issue: `TextGenerate` requires `sampling_mode` as `"on"` plus dotted child inputs like `sampling_mode.temperature`, not the earlier nested JSON object. The workflow packs and tests now use the live Comfy API shape.
- `Krea2BasePlugin.generate()` and `Krea2TurboPlugin.generate()` produced real local PNG artifacts through owned ComfyUI: `.slopperly/gpu-artifacts/smoke_16gb/krea2_base_t2i/krea2_base_t2i.png` (`1024x1024` RGB PNG, `956546` bytes) and `.slopperly/gpu-artifacts/smoke_16gb/krea2_turbo_t2i/krea2_turbo_t2i.png` (`1024x1024` RGB PNG, `931232` bytes). The Krea plugins now dynamically insert selected UI LoRAs into the Comfy graph with `LoraLoaderModelOnly`; the certified default artifacts used no selected custom LoRA.
- Anima Preview 3 files are now installed in the owned Comfy model cache: `models/diffusion_models/anima-preview3-base.safetensors` (`3.9G`), `models/text_encoders/qwen_3_06b_base.safetensors` (`1.2G`), and `models/vae/qwen_image_vae.safetensors` (`243M`).
- Owned Slopperly Comfy was live-tested on `127.0.0.1:8190` for Anima T2I and I2I with `UNETLoader`, `CLIPLoader(type=stable_diffusion)`, `VAELoader`, `CLIPTextEncode`, `EmptyLatentImage`, `LoadImage`, `ImageScale`, `VAEEncode`, `KSampler`, `VAEDecode`, `SaveImage`, and `LoraLoaderModelOnly` available in `/object_info`. The model lists included `anima-preview3-base.safetensors`, `qwen_3_06b_base.safetensors`, and `qwen_image_vae.safetensors`.
- `AnimaPlugin.generate()` produced real local T2I and I2I PNG artifacts through owned ComfyUI: `.slopperly/gpu-artifacts/smoke_16gb/anima_t2i_i2i/anima_t2i.png` (`1024x1024` RGB PNG, `312454` bytes) and `.slopperly/gpu-artifacts/smoke_16gb/anima_t2i_i2i/anima_i2i.png` (`1024x1024` RGB PNG, `599177` bytes). The Anima plugin now dynamically inserts selected UI LoRAs into either the T2I or I2I Comfy graph with `LoraLoaderModelOnly`; the certified default artifacts used no selected custom LoRA.
...
- ERNIE files are now installed in the owned Comfy model cache: `models/diffusion_models/ernie-image.safetensors` (`16067025480` bytes), `models/diffusion_models/ernie-image-turbo.safetensors` (`16067025480` bytes), `models/text_encoders/ministral-3-3b.safetensors` (`7717637511` bytes), `models/text_encoders/ernie-image-prompt-enhancer.safetensors` (`6877439999` bytes), and `models/vae/flux2-vae.safetensors` (`336213556` bytes). Current owned Comfy model cache size after the ERNIE installs is `356G`.
- Owned Slopperly Comfy was live-tested on `127.0.0.1:8190` for ERNIE Image and ERNIE Turbo with `UNETLoader`, `CLIPLoader(type=flux2)`, `VAELoader`, `TextGenerate`, `CLIPTextEncode`, `ConditioningZeroOut`, `EmptyFlux2LatentImage`, `KSampler`, `VAEDecode`, and `SaveImage` available in `/object_info`. The first live preparation exposed the same codable Comfy v3 dynamic-combo issue seen in Krea: `TextGenerate` requires `sampling_mode` as `"on"` plus dotted child inputs such as `sampling_mode.temperature` and `sampling_mode.seed`; the ERNIE workflow packs and tests now use that live API shape.
- `ErniePlugin.generate()` and `ErnieTurboPlugin.generate()` produced real local PNG artifacts through owned ComfyUI: `.slopperly/gpu-artifacts/smoke_16gb/ernie_image_t2i/ernie_image_t2i.png` (`1024x1024` RGB PNG, `1299982` bytes) and `.slopperly/gpu-artifacts/smoke_16gb/ernie_image_turbo_t2i/ernie_image_turbo_t2i.png` (`1024x1024` RGB PNG, `1331252` bytes). Comfy staged the ERNIE text encoder at `6540MB`, staged the ERNIE image model at `15322MB`, executed the base prompt in 197.51s, and executed the Turbo prompt in 20.05s.
...
- FLUX.2 Klein 4B files are now installed in the owned Comfy model cache: `models/diffusion_models/flux-2-klein-4b-fp8.safetensors` (`4070624520` bytes), `models/text_encoders/qwen_3_4b.safetensors` (`8044982048` bytes), and `models/vae/flux2-vae.safetensors` (`336213556` bytes). Current owned Comfy model cache size after the Klein 4B install is `406G`.
- Owned Slopperly Comfy was restarted and live-tested on `127.0.0.1:8190` for FLUX.2 Klein 4B with `UNETLoader`, `CLIPLoader(type=flux2)`, `VAELoader`, `CLIPTextEncode`, `ConditioningZeroOut`, `CFGGuider`, `RandomNoise`, `KSamplerSelect`, `Flux2Scheduler`, `EmptyFlux2LatentImage`, `ReferenceLatent`, `SamplerCustomAdvanced`, `VAEDecode`, `SaveImage`, and `LoraLoaderModelOnly` available in `/object_info`. The live model lists included the exact Klein diffusion, Qwen 3 4B text encoder, and FLUX.2 VAE filenames.
- `Flux2Klein4BPlugin.generate()` produced real local T2I and two-reference edit PNG artifacts through owned ComfyUI: `.slopperly/gpu-artifacts/smoke_16gb/flux2_klein_4b_t2i_edit/flux2_klein_4b_t2i.png` (`1024x1024` RGB PNG, `687191` bytes) and `.slopperly/gpu-artifacts/smoke_16gb/flux2_klein_4b_t2i_edit/flux2_klein_4b_edit.png` (`1024x1024` RGB PNG, `703338` bytes). Comfy staged the Klein text encoder at `7671MB`, the Klein diffusion model at `3882MB`, and the FLUX.2 VAE at `160MB`; after optional reference branch pruning, the clean T2I and edit prompts executed in 2.45s and 8.05s without missing-placeholder warnings.
- FLUX.2 Klein 9B files are now installed in the owned Comfy model cache: `models/diffusion_models/flux-2-klein-9b-fp8.safetensors` (`9433061528` bytes) from the public `titomatus0203/flux-2-klein-9b-fp8` mirror after the BFL FP8 repo returned gated 401 without a configured HF token, `models/text_encoders/qwen_3_8b_fp8mixed.safetensors` (`8664848742` bytes), and `models/vae/full_encoder_small_decoder.safetensors` (`249519092` bytes). Safetensors header checks passed for all three files; current owned Comfy model cache size after the Klein 9B install is `423G`.
- Owned Slopperly Comfy was live-tested on `127.0.0.1:8190` for FLUX.2 Klein 9B with `UNETLoader`, `CLIPLoader(type=flux2)`, `VAELoader`, `CLIPTextEncode`, `ConditioningZeroOut`, `CFGGuider`, `RandomNoise`, `KSamplerSelect`, `Flux2Scheduler`, `EmptyFlux2LatentImage`, `ReferenceLatent`, `SamplerCustomAdvanced`, `VAEDecode`, `SaveImage`, and `LoraLoaderModelOnly` available in `/object_info`. The workflow packs now dynamically insert selected UI LoRAs before `CFGGuider.model` and prune unused optional reference branches.
- `Flux2Klein9BPlugin.generate()` produced real local T2I and two-reference edit PNG artifacts through owned ComfyUI: `.slopperly/gpu-artifacts/smoke_16gb/flux2_klein_9b_t2i_edit/flux2_klein_9b_t2i.png` (`1024x1024` RGB PNG, `465484` bytes) and `.slopperly/gpu-artifacts/smoke_16gb/flux2_klein_9b_t2i_edit/flux2_klein_9b_edit.png` (`1024x1024` RGB PNG, `968235` bytes). Comfy staged the 9B text encoder at `8262MB`, the 9B diffusion model at `8996MB`, and the small decoder VAE at `118MB`; the manifest rerun prompts executed in 4.44s and 12.52s.
- FLUX.2 Klein 9B Schematic files are now installed in the owned Comfy model cache: `models/diffusion_models/flux-2-klein-base-9b-fp8.safetensors` (`9567278472` bytes) from the public `wissxi/FLUX.2-klein-base-9b-fp8` mirror after the official BFL FP8 source returned gated 401 without a configured HF token, `models/text_encoders/qwen_3_8b.safetensors` (`16381517176` bytes), `models/vae/flux2-vae.safetensors` (`336213556` bytes), and six schematic LoRAs under `models/loras/`: relative-depth (`165704440` bytes), surface-normal (`165704440` bytes), body-pose (`165704432` bytes), full-pose (`165704432` bytes), binary-segmentation (`165704448` bytes), and amodal-segmentation (`165704448` bytes). Safetensors header checks passed for all nine files; current owned Comfy model cache size after the schematic install is `449G`.
- Owned Slopperly Comfy was live-tested on `127.0.0.1:8190` for FLUX.2 Klein 9B Schematic with `LoadImage`, `ImageScale`, `UNETLoader`, `LoraLoaderModelOnly`, `CLIPLoader(type=flux2)`, `VAELoader`, `CLIPTextEncode`, `VAEEncode`, `ReferenceLatent`, `CFGGuider`, `RandomNoise`, `KSamplerSelect`, `Flux2Scheduler`, `EmptyFlux2LatentImage`, `SamplerCustomAdvanced`, `VAEDecode`, and `SaveImage` available in `/object_info`. Comfy staged `Flux2TEModel_` at `15622MB`, `Flux2` at `9124MB`, `AutoencoderKL` at `160MB`, attached `112` LoRA patches for each selected schematic mode, and executed the six prompts in 85.99s, 80.07s, 73.62s, 68.07s, 67.48s, and 68.42s.
- `Flux2Klein9BSchematicPlugin.generate()` produced real local PNG artifacts for all six exposed schematic modes through owned ComfyUI: DEPTH (`flux2_klein_schematic_depth.png`, `1024x1024` RGB PNG, `684K`), NORMAL (`flux2_klein_schematic_normal.png`, `1024x1024` RGB PNG, `535K`), BODY_POSE (`flux2_klein_schematic_body_pose.png`, `1024x1024` RGB PNG, `57K`), FULL_POSE (`flux2_klein_schematic_full_pose.png`, `1024x1024` RGB PNG, `55K`), BINARY_SEG (`flux2_klein_schematic_binary_seg.png`, `1024x1024` RGB PNG, `88K`), and AMODAL_SEG (`flux2_klein_schematic_amodal_seg.png`, `1024x1024` RGB PNG, `107K`). The certification manifest is `.slopperly/gpu-artifacts/smoke_16gb/flux2_klein_9b_schematic_lora/flux2_klein_schematic_manifest.json`.
...
| `image/_krea2_base.py` Krea 2 Base | LOCAL ARTIFACT PASS ONLY, NOT GGUF-COMPLIANT COMPLETION: Comfy wrapper/workflow pack uses owned Comfy with local Krea RAW Safetensors/FP8-family diffusion, Qwen3-VL text encoder, Qwen VAE, local `TextGenerate`, and `Krea2BasePlugin.generate()` produced a validated PNG artifact. | The stricter GGUF requirement is not complete for this row; install/wire a GGUF backbone if one is adopted for Krea, then rerun plugin-path artifact certification. Broader devices and any specific custom LoRA file still need their own artifact certification before being claimed. | Current local-evidence mapping covers prompt, negative prompt, resolution, frames/batch, steps, guidance/cfg, seed, sampler, scheduler, local model filenames, and selected LoRA filenames/weights. Dynamic LoRA mapping inserts `LoraLoaderModelOnly` nodes before `KSampler`. | Local evidence only: `.slopperly/gpu-artifacts/smoke_16gb/krea2_base_t2i/krea2_base_t2i.png`, RGB PNG, 1024x1024, through `Krea2BasePlugin.generate()`; GGUF completion still requires a GGUF artifact test. |
| `image/krea2_turbo.py` Krea 2 Turbo | LOCAL ARTIFACT PASS ONLY, NOT GGUF-COMPLIANT COMPLETION: Comfy wrapper/workflow pack uses owned Comfy with local Krea Turbo Safetensors/FP8-family diffusion, Qwen3-VL text encoder, Qwen VAE, local `TextGenerate`, and `Krea2TurboPlugin.generate()` produced a validated PNG artifact with the 8-step Turbo profile. | The stricter GGUF requirement is not complete for this row; install/wire a GGUF backbone if one is adopted for Krea Turbo, then rerun plugin-path artifact certification. The existing negative prompt field is preserved and surfaced in `usage_note` because the official Turbo Comfy graph uses `ConditioningZeroOut`. | Current local-evidence mapping covers prompt, resolution, frames/batch, steps default 8, guidance/cfg default 1.0, seed, sampler, scheduler, local model filenames, and selected LoRA filenames/weights. Dynamic LoRA mapping inserts `LoraLoaderModelOnly` nodes before `KSampler`; Turbo negative prompt remains an explicit official-graph limitation. | Local evidence only: `.slopperly/gpu-artifacts/smoke_16gb/krea2_turbo_t2i/krea2_turbo_t2i.png`, RGB PNG, 1024x1024, through `Krea2TurboPlugin.generate()`; GGUF completion still requires a GGUF artifact test. |
| `image/anima.py` Anima | LOCAL ARTIFACT PASS ONLY, NOT GGUF-COMPLIANT COMPLETION: Comfy wrapper and T2I/I2I workflow packs use owned Comfy with local Anima Safetensors diffusion, Qwen 0.6B text encoder, Qwen VAE, and `AnimaPlugin.generate()` produced validated local T2I and I2I PNG artifacts. | The stricter GGUF requirement is not complete for this row; install/wire a GGUF backbone if one is adopted for Anima, then rerun T2I and I2I plugin-path artifact certification. Broader devices and any specific custom LoRA file still need their own artifact certification before being claimed. | Current local-evidence mapping covers prompt, negative prompt, selected image strip upload for I2I, resolution, frames/batch, steps, guidance/cfg, strength-to-denoise, seed, sampler, scheduler, local model filenames, and selected LoRA filenames/weights. Dynamic LoRA mapping inserts `LoraLoaderModelOnly` nodes before the T2I or I2I `KSampler`. | Local evidence only: `.slopperly/certification/smoke_16gb/anima_t2i_i2i.json`, manifest plus `anima_t2i.png` and `anima_i2i.png`, both RGB PNG 1024x1024 through `AnimaPlugin.generate()`; GGUF completion still requires a GGUF artifact test. |
...
| `image/ernie.py` ERNIE Image | LOCAL ARTIFACT PASS ONLY, NOT GGUF-COMPLIANT COMPLETION: Comfy wrapper and `ernie_image_t2i` workflow pack use owned Comfy with local ERNIE Safetensors diffusion, Ministral text encoder, ERNIE prompt enhancer, FLUX.2 VAE, and `ErniePlugin.generate()` produced a validated PNG artifact. | The stricter GGUF requirement is not complete for this row; install/wire a GGUF backbone if one is adopted for ERNIE, then rerun plugin-path artifact certification. Broader devices and future ERNIE profiles still need their own artifact certification before exposure. | Current local-evidence mapping covers prompt, negative prompt, resolution, frames/batch, steps, guidance/cfg, seed, sampler, scheduler, local model filenames, and local `TextGenerate` prompt-enhancer settings. | Local evidence only: `.slopperly/certification/smoke_16gb/ernie_image_t2i.json`, RGB PNG 1024x1024 through `ErniePlugin.generate()`; GGUF completion still requires a GGUF artifact test. |
| `image/ernie_turbo.py` ERNIE Turbo | LOCAL ARTIFACT PASS ONLY, NOT GGUF-COMPLIANT COMPLETION: Comfy wrapper and `ernie_image_turbo_t2i` workflow pack use owned Comfy with local ERNIE Turbo Safetensors diffusion, Ministral text encoder, ERNIE prompt enhancer, FLUX.2 VAE, and `ErnieTurboPlugin.generate()` produced a validated PNG artifact with the 8-step Turbo profile. | The stricter GGUF requirement is not complete for this row; install/wire a GGUF backbone if one is adopted for ERNIE Turbo, then rerun plugin-path artifact certification. The existing negative prompt field is preserved and surfaced in `usage_note` because the official Turbo Comfy graph uses `ConditioningZeroOut`. | Current local-evidence mapping covers prompt, resolution, frames/batch, steps default 8, guidance/cfg default 1.0, seed, sampler, scheduler, local model filenames, and local `TextGenerate` prompt-enhancer settings; Turbo negative prompt remains an explicit official-graph limitation. | Local evidence only: `.slopperly/certification/smoke_16gb/ernie_image_turbo_t2i.json`, RGB PNG 1024x1024 through `ErnieTurboPlugin.generate()`; GGUF completion still requires a GGUF artifact test. |
...
| `image/flux2_klein_4b.py` FLUX.2 Klein 4B | LOCAL ARTIFACT PASS ONLY, NOT GGUF-COMPLIANT COMPLETION: native FP8 Comfy workflow packs use owned Comfy with `flux-2-klein-4b-fp8.safetensors`, `qwen_3_4b.safetensors`, FLUX.2 VAE, and `Flux2Klein4BPlugin.generate()` produced validated T2I and two-reference edit PNG artifacts. | The stricter GGUF requirement is not complete for this row; a Klein 4B GGUF backbone must be installed/wired and T2I/edit artifacts rerun before claiming GGUF completion. The official `ReferenceLatent` edit graph has no denoise/strength or mask input; specific user-selected LoRA files still need their own artifact proof before being claimed as compatible. | Current local-evidence mapping covers prompt, image strip/edit mode, up to three compressed reference inputs, resolution, frames/batch, steps, guidance/cfg, seed, sampler, local model filenames, selected LoRA filenames/weights, and optional reference branch pruning. Dynamic LoRA mapping inserts `LoraLoaderModelOnly` nodes before `CFGGuider.model`. | Local evidence only: `.slopperly/certification/smoke_16gb/flux2_klein_4b_t2i_edit.json`, manifest plus `flux2_klein_4b_t2i.png` and `flux2_klein_4b_edit.png`, both RGB PNG 1024x1024 through `Flux2Klein4BPlugin.generate()`; GGUF completion still requires a GGUF artifact test. |
| `image/flux2_klein_9b.py` FLUX.2 Klein 9B | LOCAL ARTIFACT PASS ONLY, NOT GGUF-COMPLIANT COMPLETION: native FP8 Comfy workflow packs use owned Comfy with `flux-2-klein-9b-fp8.safetensors`, `qwen_3_8b_fp8mixed.safetensors`, `full_encoder_small_decoder.safetensors`, and `Flux2Klein9BPlugin.generate()` produced validated T2I and two-reference edit PNG artifacts. | The stricter GGUF requirement is not complete for this row; a Klein 9B GGUF backbone must be installed/wired and T2I/edit artifacts rerun before claiming GGUF completion. The official `ReferenceLatent` edit graph has no denoise/strength or mask input; specific user-selected LoRA files still need their own artifact proof before being claimed as compatible. | Current local-evidence mapping covers prompt, image strip/edit mode, up to three compressed reference inputs, resolution, frames/batch, steps, guidance/cfg, seed, sampler, local model filenames, selected LoRA filenames/weights, and optional reference branch pruning. Dynamic LoRA mapping inserts `LoraLoaderModelOnly` nodes before `CFGGuider.model`. | Local evidence only: `.slopperly/certification/smoke_16gb/flux2_klein_9b_t2i_edit.json`, manifest plus `flux2_klein_9b_t2i.png` (`465484` bytes) and `flux2_klein_9b_edit.png` (`968235` bytes), both RGB PNG 1024x1024 through `Flux2Klein9BPlugin.generate()`; GGUF completion still requires a GGUF artifact test. |
| `image/flux2_klein_9b_schematic.py` schematic LoRA | LOCAL ARTIFACT PASS ONLY, NOT GGUF-COMPLIANT COMPLETION: native FLUX.2 Klein 9B FP8 base plus schematic LoRA Comfy workflow pack uses owned Comfy with the base 9B FP8 diffusion, full Qwen 3 8B text encoder, FLUX.2 VAE, and all six schematic LoRA files; `Flux2Klein9BSchematicPlugin.generate()` produced validated local PNG artifacts for every exposed schematic mode. | The stricter GGUF requirement is not complete for this row; a schematic-compatible Klein 9B GGUF backbone must be installed/wired and all six mode artifacts rerun before claiming GGUF completion. The plugin has no negative-prompt UI section, so the upstream fixed negative prompt remains an internal workflow constant. | Current local-evidence mapping covers required image strip upload, prompt, schematic mode selector to exact LoRA filename, segmentation target for BINARY_SEG/AMODAL_SEG prompts, frames/batch, steps, guidance/cfg, seed, base model, text encoder, VAE, and per-run output path. | Local evidence only: `.slopperly/certification/smoke_16gb/flux2_klein_9b_schematic_lora.json`, manifest plus six RGB PNG 1024x1024 artifacts for DEPTH, NORMAL, BODY_POSE, FULL_POSE, BINARY_SEG, and AMODAL_SEG through `Flux2Klein9BSchematicPlugin.generate()`; GGUF completion still requires a GGUF artifact test. |
...
| `video/ltx2.py` LTX2 19B | DONE ON SPEC for the certified short I2V `smoke_16gb` Q5 profile: the production plugin now routes through owned ComfyUI with `ltx23_i2v`, `UnetLoaderGGUFDisTorch2MultiGPU`, LTX text encoder/connector, video/audio VAEs, and `LTX2Plugin.generate()` produced a validated MP4 artifact. | No remaining production blocker for the certified I2V Q5 profile; LTX T2V, 1080p, arbitrary project LoRA injection, and staged extend/lipsync/multi/IC workflows still need separate ports and real artifact certification before exposure. | Prompt, negative prompt, selected image strip or first frame from selected video, 1280x720 UI request mapped to certified 1280x704, frames, fps=24, seed, strength, exact Q5 GGUF model, text encoder, connector, audio/video VAEs, and committed distilled LoRA are patched into the owned Comfy workflow; steps/guidance stay visible with usage notes because the graph uses a fixed `ManualSigmas` schedule and `BasicGuider`. | PASS: `.slopperly/certification/smoke_16gb/ltx23_i2v_q5_gguf.json`, MP4 1280x704, 24fps, 17 frames, 0.708008s, AAC audio present, through `LTX2Plugin.generate()`. |
...
- Current parity status: `image/anima.py` is DONE ON SPEC for the `smoke_16gb` profile. Text-to-image and img2img each have real owned-Comfy `AnimaPlugin.generate()` PNG artifacts, exact local Anima model files, dropdown PASS certification, and no remaining production blocker for the certified profile.
...
- Current parity status: `image/ernie.py` and `image/ernie_turbo.py` are DONE ON SPEC for the `smoke_16gb` profile. Base and Turbo each have real owned-Comfy `ModelPlugin.generate()` PNG artifacts, exact ERNIE model files, dropdown PASS certification, and no remaining production blocker for those certified profiles. ERNIE Turbo's negative prompt field remains preserved with an explicit usage note because the official Turbo graph uses `ConditioningZeroOut`.
...
- Current parity status: `image/_krea2_base.py` and `image/krea2_turbo.py` are DONE ON SPEC for the `smoke_16gb` profile. Base and Turbo each have real owned-Comfy `ModelPlugin.generate()` PNG artifacts, exact Krea model files, dropdown PASS certification, and no remaining production blocker for the certified default profiles. Krea Turbo's negative prompt field remains preserved with an explicit usage note because the official Turbo graph uses `ConditioningZeroOut`; specific user-selected LoRA files still need their own artifact proof before being claimed as compatible.
...
- Evidence: `python -m pytest tests/unit tests/integration -q`, `python -m slopperly.audit.model_registry`, `python -m slopperly.audit.workflow_packs`, `python -m slopperly.audit.no_cloud`, and `python -m slopperly.audit.dropdown_certification --profile smoke_16gb --report-only` passed after Krea certification; dropdown certification reports 17 passed and 23 blocked entries.
...
- Current parity status: `image/flux2_klein_4b.py` is DONE ON SPEC for the certified `smoke_16gb` profile. Text-to-image and two-reference edit each have real owned-Comfy `Flux2Klein4BPlugin.generate()` PNG artifacts, exact local Klein 4B model files, dynamic selected-LoRA graph insertion, dropdown PASS certification, and no remaining production blocker for the certified 16GB-default profile. Masked inpaint and image-strength/denoise remain documented official-graph limitations until a separate local workflow exposes those controls and passes artifact certification.
...
- Evidence: owned Klein files are present in `.slopperly/runtimes/ComfyUI/models`: `diffusion_models/flux-2-klein-4b-fp8.safetensors` (`4070624520` bytes), `text_encoders/qwen_3_4b.safetensors` (`8044982048` bytes), and `vae/flux2-vae.safetensors` (`336213556` bytes). Owned Comfy model cache size after this install is `406G`.
...
- Evidence: `python -m slopperly.audit.dropdown_certification --profile smoke_16gb --report-only` reports `PASS flux2_klein_4b_t2i_edit`; dropdown certification reports 23 passed and 17 blocked entries.
...
### 2026-06-28 FLUX.2 Klein 9B Comfy certification block

- Current parity status: `image/flux2_klein_9b.py` is DONE ON SPEC for the certified `smoke_16gb` profile. Text-to-image and two-reference edit each have real owned-Comfy `Flux2Klein9BPlugin.generate()` PNG artifacts, exact local Klein 9B model files, dynamic selected-LoRA graph insertion, optional reference branch pruning, dropdown PASS certification, and no remaining production blocker for the certified profile. Masked inpaint and image-strength/denoise remain documented official-graph limitations until a separate local workflow exposes those controls and passes artifact certification.
...
- Evidence: `python -m slopperly.audit.dropdown_certification --profile smoke_16gb --report-only` reports `PASS flux2_klein_9b_t2i_edit`; dropdown certification now reports 24 passed and 16 blocked entries.
...
### 2026-06-28 FLUX.2 Klein 9B Schematic LoRA Comfy certification block

- Current parity status: `image/flux2_klein_9b_schematic.py` is DONE ON SPEC for the certified `smoke_16gb` profile. Every exposed schematic mode has real owned-Comfy `Flux2Klein9BSchematicPlugin.generate()` PNG evidence, exact local base/text/VAE/LoRA files, dropdown PASS certification, and no remaining production blocker for that certified six-mode profile.
...
- Implemented: `flux2_klein_9b_schematic_lora` workflow pack is committed with API/editable workflow JSON, schema, model manifest, smoke payload, and README. The model registry and workflow manifest now point the base 9B FP8 artifact to the public `wissxi/FLUX.2-klein-base-9b-fp8` mirror used for the local install; the original BFL FP8 source returned gated 401 without a configured HF token.
- Implemented: the workflow uses FLUX.2 Klein 9B base, matching the upstream schematic LoRA README, with Comfy core `LoadImage`, `ImageScale`, `UNETLoader`, `LoraLoaderModelOnly`, `CLIPLoader`, `VAELoader`, `CLIPTextEncode`, `VAEEncode`, `ReferenceLatent`, `CFGGuider`, `RandomNoise`, `KSamplerSelect`, `Flux2Scheduler`, `EmptyFlux2LatentImage`, `SamplerCustomAdvanced`, `VAEDecode`, and `SaveImage`.
...
- Evidence: the built-in Hugging Face downloader stalled inside `httpx` body streaming before creating target files; the transfer was cleanly interrupted and replaced with explicit resumable HTTP downloads. This was a codable install tactic, not a final blocker.
- Evidence: owned schematic files are present in `.slopperly/runtimes/ComfyUI/models`: `diffusion_models/flux-2-klein-base-9b-fp8.safetensors` (`9567278472` bytes), `text_encoders/qwen_3_8b.safetensors` (`16381517176` bytes), `vae/flux2-vae.safetensors` (`336213556` bytes), and six LoRAs in `loras/` with exact sizes `165704440`, `165704440`, `165704432`, `165704432`, `165704448`, and `165704448` bytes. All nine files passed safetensors header reads; owned Comfy model cache size after this install is `449G`.
- Evidence: `HF_HUB_DISABLE_XET=1 .slopperly/runtimes/comfy-venv/bin/python -m slopperly.models.download --model flux2_klein_9b_schematic_lora --cache-root .slopperly/runtimes/ComfyUI --profile smoke_16gb --accept-licenses` reports all nine schematic artifacts already cached and `0 blocked`.
- Evidence: owned Slopperly ComfyUI was started on `http://127.0.0.1:8190` with API nodes disabled, CUDA 13 PyTorch, DynamicVRAM, and core FLUX.2 Klein/LoRA nodes available. Comfy logs for the six-mode run showed `Flux2TEModel_` staged at `15622MB`, `Flux2` staged at `9124MB`, `AutoencoderKL` staged at `160MB`, and `112` LoRA patches attached for each selected schematic mode.
- Evidence: `SLOPPERLY_COMFYUI_URL=http://127.0.0.1:8190 python -m pytest tests/gpu/test_flux2_klein_schematic.py --device cuda --profile smoke_16gb --runtime-timeout 30 -s` passed 1 manifest-producing test in 446.41s through `Flux2Klein9BSchematicPlugin.generate()`, running DEPTH, NORMAL, BODY_POSE, FULL_POSE, BINARY_SEG, and AMODAL_SEG.
- Evidence: `.slopperly/certification/smoke_16gb/flux2_klein_9b_schematic_lora.json` is PASS for `.slopperly/gpu-artifacts/smoke_16gb/flux2_klein_9b_schematic_lora/flux2_klein_schematic_manifest.json`; the manifest points to all six generated PNG artifacts.
- Evidence: generated artifacts validate as RGB PNGs at 1024x1024: `flux2_klein_schematic_depth.png` (`684K`), `flux2_klein_schematic_normal.png` (`535K`), `flux2_klein_schematic_body_pose.png` (`57K`), `flux2_klein_schematic_full_pose.png` (`55K`), `flux2_klein_schematic_binary_seg.png` (`88K`), and `flux2_klein_schematic_amodal_seg.png` (`107K`).
...
- Evidence: `python -m slopperly.audit.dropdown_certification --profile smoke_16gb --report-only` reports `PASS flux2_klein_9b_schematic_lora`; dropdown certification now reports 25 passed and 15 blocked entries.
...
- Current parity status: `video/ltx2.py` is DONE ON SPEC for the certified short I2V `smoke_16gb` Q5 profile. The existing LTX image/video-strip UI path now calls `LTX2Plugin.generate()`, routes through owned ComfyUI, loads the LTX 2.3 distilled Q5 GGUF backbone, and has a real RTX 4090 MP4 PASS artifact. This does not certify LTX T2V, 1080p, extend, lipsync, multi-anchor, or IC-LoRA workflows; those rows still require their own ports and artifact tests.
...
- Not done - next action required: port and artifact-certify LTX T2V, LTX 1080p, `video/ltx23_extend.py`, `video/ltx23_lipsync.py`, `video/ltx23_multi.py`, and `video/ltx23_multi_ic_lora.py` through owned ComfyUI before claiming those LTX workflows complete.
...
- Implemented: `slopperly/config/models.yaml` now explicitly marks Qwen Image 2512, Qwen Image Edit 2511, FLUX.2 Dev, Wan TI2V-5B, and LTX 2.3 I2V as GGUF-backed certified entries. It marks FLUX.2 Klein 4B, FLUX.2 Klein 9B, FLUX.2 Klein 9B Schematic, Anima, ERNIE Image, ERNIE Turbo, Krea 2 Base, and Krea 2 Turbo as GGUF-required but currently safetensors/native/FP8 evidence only, with the next action recorded for each row.
...
- Evidence: `python -m slopperly.audit.dropdown_certification --profile smoke_16gb --report-only` now reports 20 passed and 21 blocked. The rows newly blocked by the GGUF gate are `flux2_klein_4b_t2i_edit`, `flux2_klein_9b_t2i_edit`, `flux2_klein_9b_schematic_lora`, `anima_t2i_i2i`, `ernie_image_t2i`, `ernie_image_turbo_t2i`, `krea2_base_t2i`, and `krea2_turbo_t2i`.
- Not done - next action required: install or create GGUF-backed workflow profiles for those eight image rows where a suitable GGUF backbone exists, then rerun the existing plugin-path artifact tests. Rows without a real GGUF artifact must stay blocked or hidden rather than claimed complete.

...
- **GGUF-COMPLIANT PASS NOW:** only `image/flux2_dev.py` through `flux2_dev_gguf_quality`, `video/wan_ti2v_5b.py` through `wan22_ti2v_5b_720p24_gguf`, and `text/moviigen_rewriter.py` through `llamacpp_prompt_rewriter` currently have PASS records whose primary inference backbone is GGUF. FLUX.2 Dev and Wan use GGUF diffusion/video backbones with Safetensors text encoders where the text-encoder rule above permits them; MoviiGen uses the local Qwen2.5 Q5 GGUF served by llama.cpp.
...
- **GGUF WORK STILL NOT DONE:** `image/qwen_image.py` (`qwen_image_2512_t2i_gguf`) and `image/qwen_image_edit.py` (`qwen_image_edit_2511_multi_gguf`) are registered GGUF targets but still blocked for lack of real certification. Original `video/wan_t2v.py` and `video/wan_i2v.py` still need Wan A14B high/low-noise GGUF Comfy workflows and real T2V/I2V artifacts. The LTX rows have Q5 GGUF assets in the owned cache, but the production plugin paths still need Comfy workflow ports and real artifacts before any GGUF completion claim.

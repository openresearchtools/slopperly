...
- **DONE ON SPEC:** `audio/_stable_audio_3.py` Stable Audio 3, `audio/ace_step.py` ACE-Step, `audio/foundation_music.py` Foundation-1, `audio/mmaudio.py` MMAudio, and the certified four-stem path of `audio/stem_split.py` Stem Splitter from baseline commit `50cf377bf11349685010acb726a5e2b2e3cb9962` now have real local `ModelPlugin.generate()` PASS artifacts through owned Slopperly ComfyUI on the RTX 4090 profile. Stem Splitter's old six-stem option remains a documented blocked mode until a separate six-stem local workflow passes artifact certification.

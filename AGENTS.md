...
- **DONE ON SPEC FOR NON-GGUF-RUNTIME ROWS:** `audio/_stable_audio_3.py` Stable Audio 3, `audio/ace_step.py` ACE-Step, `audio/foundation_music.py` Foundation-1, `audio/mmaudio.py` MMAudio, `audio/chatterbox.py` Chatterbox, `audio/chatterbox_turbo.py` Chatterbox Turbo, `audio/chatterbox_multilingual.py` Chatterbox Multilingual, `audio/omnivoice.py` OmniVoice, `audio/moss_tts.py` MOSS-TTS, `text/faster_whisper_transcribe.py` STT, `text/marlin_video_captions.py` Marlin Video Captions, `text/florence2.py` Florence2, `image/birefnet.py` BiRefNet, `image/maxine_vsr.py` Local Image VSR, `image/omnigen.py` OmniGen native Comfy multi-image, and the certified four-stem path of `audio/stem_split.py` Stem Splitter from baseline commit `50cf377bf11349685010acb726a5e2b2e3cb9962` have real local `ModelPlugin.generate()` PASS artifacts on the RTX 4090 profile through their required local runtimes. OmniGen is a `MIGRATE_NATIVE_COMFY` row, not GGUF evidence. Stem Splitter's old six-stem option and Chatterbox's true two-audio cross-speaker VC remain documented modes until separate UI/workflow certification exists.
...
- OmniGen native-Comfy assets are now present in the owned cache under `.slopperly/runtimes/ComfyUI/models/LLM/OmniGen-v1/`, including `model.safetensors` (`15501299112` bytes), local config/tokenizer files, and `vae/diffusion_pytorch_model.safetensors` (`334643268` bytes). This is a `MIGRATE_NATIVE_COMFY` profile, not GGUF evidence.
- The owned OmniGen custom-node code is pinned from VectorSpaceLab/OmniGen commit `d4752019ae16741dc3801fd4989432aad4c81a4f` through the Slopperly Comfy installer, with local-only/download guards plus reproducible patches for Phi3 rope config, current Phi3 decoder API, OmniGen cache API compatibility, first-pass cache return shape, Memory Priority CUDA deferral, and local VAE fallback. `slopperly/runtime/comfy/nodes.lock.yaml` records the `peft` dependency required by the live node.
- Owned Slopperly Comfy was live-tested on `127.0.0.1:8190` for OmniGen with API nodes disabled, CUDA 13 PyTorch, DynamicVRAM, `ailab_OmniGen`, `LoadImage`, and `SaveImage` exposed in `/object_info`; `OmniGenPlugin.generate()` uploaded three local reference images, mapped prompt placeholders without nested raw placeholder tags, and produced `.slopperly/gpu-artifacts/smoke_16gb/omnigen_v1_multi_image/omnigen_multi_image.png`, a 512x512 RGB PNG (`172K`). The PASS record `.slopperly/certification/smoke_16gb/omnigen_v1_multi_image.json` records the three reference fixtures, workflow pack, runtime URL, and validation result.
...
|---|---|---|---|
| Indexed Comfy media schema | Schema helper scaffold exists for indexed fields and media uploads. Qwen edit and OmniGen now have separate later real PASS records that use this plumbing. | The schema itself proves no model parity; LTX staged/multi and any future indexed consumer still need their own plugin-path artifacts. | Remaining indexed workflows must pass real multi-reference artifact tests through their own `ModelPlugin.generate()` paths. |
...
| `image/omnigen.py` OmniGen | DONE ON SPEC for the certified `smoke_16gb` native-Comfy profile: `OmniGenPlugin.generate()` routes through owned ComfyUI `omnigen_v1_multi_image`, loads the local `Shitao/OmniGen-v1` snapshot through pinned local OmniGen code, uploads three reference images, and produced a validated PNG artifact. | No remaining production blocker for the certified native-Comfy multi-image profile. This is `MIGRATE_NATIVE_COMFY`, not GGUF evidence, and broader device profiles still need their own certification. | Up to three image refs, image placeholder prompt text, optional slot pruning, output width/height, frames/batch, steps, guidance, image guidance, seed, max input image size, memory management, exact local model path, and per-run save prefix are mapped through the owned Comfy workflow. | PASS: `.slopperly/certification/smoke_16gb/omnigen_v1_multi_image.json`; RGB PNG 512x512 at `.slopperly/gpu-artifacts/smoke_16gb/omnigen_v1_multi_image/omnigen_multi_image.png` through `OmniGenPlugin.generate()` with three local reference fixtures. |
...
- Current parity status: SCAFFOLD ONLY FOR THE SHARED SCHEMA. Indexed patching is required by Qwen edit, OmniGen, and LTX staged/multi workflows; Qwen edit and OmniGen now have separate later plugin-path PASS artifacts, while LTX staged/multi still needs its own real artifacts.
...
- Not done - install/test required: remaining indexed consumers, especially LTX staged/multi, still require owned Comfy runtime execution, model downloads where missing, full UI mapping, and GPU artifact certification through their own plugin paths.
...
- Current parity status: `image/omnigen.py` is DONE ON SPEC for the certified `smoke_16gb` native-Comfy profile. The existing triple prompt/image UI path calls `OmniGenPlugin.generate()`, routes through owned ComfyUI `omnigen_v1_multi_image`, loads the local `Shitao/OmniGen-v1` snapshot through pinned local OmniGen code, uploads three real reference images, and has a real RTX 4090 PNG PASS artifact. This is `MIGRATE_NATIVE_COMFY`, not GGUF evidence.

- Implemented: `image/omnigen.py` routes the legacy `Shitao/OmniGen-v1-diffusers` plugin through the local Comfy workflow gateway instead of direct Torch/Diffusers `OmniGenPipeline` execution in the add-on process.
- Implemented: the existing triple prompt/image UI is preserved; the wrapper resolves the same scene strip pickers to local paths, preserves `img_guidance_scale`, maps output size, frames/batch, steps, guidance, seed, max input image size, and memory-management controls, and returns the existing PNG artifact path shape.
- Implemented: the prompt composer now appends plain `image_1` / `image_2` / `image_3` references only when the user prompt does not already mention that image, avoiding nested raw `<img><|image_n|></img>` tags that broke the live node parser.
- Implemented: single-reference mode keeps `use_input_image_size_as_output=True`, while multi-reference mode disables that node option so requested output dimensions are honored instead of being forced to a reference image size.
- Implemented: `omnigen_v1_multi_image` workflow pack is committed with API/editable workflow JSON, schema, model manifest, smoke payload, and README. The pack supports optional slot pruning for empty reference slots and exact indexed uploads for populated slots.
- Implemented: `slopperly/runtime/comfy/nodes.lock.yaml` asserts `ailab_OmniGen` from the pinned OmniGen node path and records the required `peft` Python extra.
- Implemented: the Comfy installer reproducibly prepares pinned OmniGen code at commit `d4752019ae16741dc3801fd4989432aad4c81a4f`, disables first-run downloads, and applies local compatibility patches for Phi3 rope config, current Phi3 decoder API, OmniGen cache API shape, first-pass cache return shape, Memory Priority CUDA deferral, and local VAE loading.
- Evidence: owned model files are present under `.slopperly/runtimes/ComfyUI/models/LLM/OmniGen-v1/`, including `model.safetensors` (`15501299112` bytes), local config/tokenizer files, and `vae/diffusion_pytorch_model.safetensors` (`334643268` bytes).
- Evidence: owned Slopperly ComfyUI on `http://127.0.0.1:8190` exposed `ailab_OmniGen`, `LoadImage`, and `SaveImage` in `/object_info` after the installer patches and node dependency were present.
- Evidence: the first real live attempts exposed codable issues, not final blockers: Phi3 rope config placement, current Phi3 decoder `position_embeddings`, nested OmniGen prompt tags, missing legacy `DynamicCache.key_cache`, and cropped first-pass K/V return shapes. Each was fixed in code or installer patch and rerun before the final PASS.
- Evidence: `SLOPPERLY_COMFYUI_URL=http://127.0.0.1:8190 python -m pytest tests/gpu/test_omnigen.py --device cuda --profile smoke_16gb --runtime-timeout 30 -s` passed 1 test in 55.26s through `OmniGenPlugin.generate()` using three local reference images.
- Evidence: `.slopperly/certification/smoke_16gb/omnigen_v1_multi_image.json` is PASS for `.slopperly/gpu-artifacts/smoke_16gb/omnigen_v1_multi_image/omnigen_multi_image.png`; validation records PNG/RGB, 512x512, no alpha, and metadata records the three reference fixtures, runtime URL, and workflow pack.
- Evidence: generated artifact `.slopperly/gpu-artifacts/smoke_16gb/omnigen_v1_multi_image/omnigen_multi_image.png` is a 172K PNG image data file, 512 x 512, 8-bit/color RGB, non-interlaced.
- Evidence: focused support checks passed for the OmniGen installer/model-download compatibility patches, plugin-path integration, workflow optional-slot pruning, and compile checks. The final dropdown audit reports `PASS omnigen_v1_multi_image`, 43 passed entries, and 1 blocked entry.
...
- Evidence: at the time of this Nucleus block, `python -m slopperly.audit.dropdown_certification --profile smoke_16gb --report-only` reported `PASS nucleus_image_t2i`, 42 passed entries, and 2 blocked entries. The later OmniGen certification block supersedes that count.
- Superseded next action: after the later OmniGen plugin-path artifact passed, only `local_video_vsr_upscale` remains blocked in current dropdown certification.

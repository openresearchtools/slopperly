...
- **DONE ON SPEC:** `audio/_stable_audio_3.py` Stable Audio 3, `audio/ace_step.py` ACE-Step, `audio/foundation_music.py` Foundation-1, `audio/mmaudio.py` MMAudio, `audio/chatterbox.py` Chatterbox, `audio/chatterbox_turbo.py` Chatterbox Turbo, and the certified four-stem path of `audio/stem_split.py` Stem Splitter from baseline commit `50cf377bf11349685010acb726a5e2b2e3cb9962` now have real local `ModelPlugin.generate()` PASS artifacts through owned Slopperly ComfyUI on the RTX 4090 profile. Stem Splitter's old six-stem option and Chatterbox's true two-audio cross-speaker VC remain documented blocked modes until separate UI/workflow certification exists.
...
- **CERTIFICATION SUMMARY:** `python -m slopperly.audit.dropdown_certification --profile smoke_16gb --report-only` now reports `PASS wan22_ti2v_5b_720p24_gguf`, `PASS audio_stem_split_demucs`, `PASS mmaudio_video_to_audio`, `PASS stable_audio_3_medium_base`, `PASS ace_step_15_music`, `PASS foundation1_music_loop`, `PASS chatterbox_tts_vc_comfy`, `PASS chatterbox_turbo_tts_comfy`, and 32 blocked entries still requiring the next model install/runtime/artifact test.
...
- Existing user Comfy custom nodes include `ComfyUI-GGUF`, `ComfyUI-MultiGPU`, `WhatDreamsCost-ComfyUI`, and `ComfyUI-KJNodes`; owned Slopperly Comfy has now been live-tested with `comfyui_gguf`, `video_helper_suite`, `mmaudio`, `audio_separation`, `foundation_1`, and `chatterbox` for the certified blocks. Missing future node packs must be installed into `.slopperly/runtimes/ComfyUI/custom_nodes`, not relied on from the user Comfy tree.
...
- Current parity status: `audio/chatterbox_turbo.py` is DONE ON SPEC for the `smoke_16gb` profile. Turbo prompt-only and Turbo ref-audio each have real owned-Comfy `ChatterboxTurboPlugin.generate()` WAV artifacts, exact Chatterbox Turbo model files, dropdown PASS certification, and no remaining production blocker for that certified profile. `audio/chatterbox_multilingual.py` is still NOT DONE ON SPEC; multilingual prompt-only and multilingual ref-audio still need owned Comfy model artifacts, UI control mapping, real WAV artifacts, and dropdown PASS certification.
...
- Scaffold only: `chatterbox_multilingual_tts_comfy` and `chatterbox_multilingual_ref_tts_comfy` workflow packs are committed with API/editable workflow JSON, schemas, model manifests, smoke payloads, and READMEs but still lack real RTX artifact certification.
...
- Scaffold only: `chatterbox_multilingual_tts_comfy` is registered in `slopperly/config/models.yaml` with legacy alias `ChatterboxMultilingual` but still requires real model install and artifact certification.
...
- Evidence: integration coverage calls both plugin `load()`/`generate()` paths against a loopback fake Comfy server under the local-network guard and verifies plain/ref graph patching plus WAV artifact return for Turbo and FLAC artifact collection for Multilingual scaffold.
...
- Evidence: `python -m slopperly.audit.model_registry`, `python -m slopperly.audit.workflow_packs`, `python -m pytest tests/unit tests/integration`, `python -m slopperly.audit.no_cloud`, and `python -m slopperly.audit.dropdown_certification --profile smoke_16gb --report-only` passed after this Turbo certification; dropdown certification reports 8 passed and 32 blocked entries.
- Not done - install/test required: real RTX 4090 Chatterbox Multilingual artifact certification still requires owned ComfyUI running with ComfyUI_Fill-ChatterBox, core audio nodes, multilingual Chatterbox artifacts installed under `models/chatterbox/chatterbox_multilingual/`, and real prompt-only plus ref-audio WAV artifacts through `ChatterboxMultilingualPlugin.generate()`.

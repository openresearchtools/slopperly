...
- **DONE ON SPEC:** `audio/_stable_audio_3.py` Stable Audio 3, `audio/ace_step.py` ACE-Step, `audio/foundation_music.py` Foundation-1, `audio/mmaudio.py` MMAudio, `audio/chatterbox.py` Chatterbox, `audio/chatterbox_turbo.py` Chatterbox Turbo, `audio/chatterbox_multilingual.py` Chatterbox Multilingual, `audio/omnivoice.py` OmniVoice, and the certified four-stem path of `audio/stem_split.py` Stem Splitter from baseline commit `50cf377bf11349685010acb726a5e2b2e3cb9962` now have real local `ModelPlugin.generate()` PASS artifacts on the RTX 4090 profile. The Comfy-backed entries run through owned Slopperly ComfyUI; OmniVoice runs through the owned local vLLM-Omni runtime. Stem Splitter's old six-stem option and Chatterbox's true two-audio cross-speaker VC remain documented blocked modes until separate UI/workflow certification exists.
...
- `.slopperly/vllm-omni-venv` is now installed with matching `vllm-omni==0.22.0` and `vllm==0.22.0`; the installer applies a Slopperly patch to the local OmniVoice pipeline so request `extra_params` can drive the existing steps and guidance UI controls (`num_step` and `guidance_scale`) instead of leaving them as fixed config-only values.
...
- Remaining vLLM-Omni functions require one local speech model per server instance; MOSS-TTS still needs its own local model install/start and real artifact test.

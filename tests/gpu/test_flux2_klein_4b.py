...
def test_flux2_klein_4b_t2i_and_edit(
    gpu_cert,
    plugin_loader,
    base_models,
    repo_root,
    ...
    t2i = _run_flux2_klein_case(
        ...
        seed=420041,
        filename="flux2_klein_4b_t2i.png",
        mode="t2i",
    )
    edit = _run_flux2_klein_case(
        ...
        seed=420042,
        filename="flux2_klein_4b_edit.png",
        mode="edit",
        sequence_scene_factory=sequence_scene_factory,
    )

    manifest_path = gpu_cert.artifact_path(LOGICAL_NAME, "flux2_klein_4b_manifest.json")
    manifest = {
        "t2i": t2i,
        "edit": edit,
    }
...
def _run_flux2_klein_case(
    ...
    *,
    ...
    workflow_pack = gpu_cert.require_file(
        ...
        f"FLUX.2 Klein 4B Comfy {mode} workflow API graph",
    )
    image = None
    ...
    if mode == "edit":
        if sequence_scene_factory is None:
            gpu_cert.block(LOGICAL_NAME, "sequence scene factory is required for FLUX.2 Klein 4B edit test")
    ...
    try:
        ...
        gpu_cert.block(LOGICAL_NAME, f"FLUX.2 Klein 4B {mode} Comfy plugin path runtime error: {exc}")
    except ArtifactValidationError as exc:
        gpu_cert.fail(LOGICAL_NAME, f"FLUX.2 Klein 4B {mode} PNG validation failed: {exc}")

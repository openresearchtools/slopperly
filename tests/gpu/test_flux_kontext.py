...
LOGICAL_NAME = "flux_kontext_edit"
WORKFLOW_ID = "flux_kontext_edit"


def test_flux_kontext_edit(gpu_cert, plugin_loader, base_models, repo_root):
    ...
    source = gpu_cert.require_file(
        ...
        "FLUX Kontext source image fixture",
    )

    module = plugin_loader("image", "flux_kontext")
    plugin = module.FluxKontextPlugin()
    output_path = gpu_cert.artifact_path(LOGICAL_NAME, "flux_kontext_edit.png")
    module.solve_path = lambda generated: str(output_path)

    inputs = base_models.ModelInputs(
        prompt="turn the subject red while preserving the source layout",
        image=str(source),
        width=1024,
        height=1024,
        steps=28,
    ...
    try:
        ...
        gpu_cert.block(LOGICAL_NAME, f"FLUX Kontext Comfy plugin path runtime error: {exc}")
    except ArtifactValidationError as exc:
        gpu_cert.fail(LOGICAL_NAME, f"FLUX Kontext PNG validation failed: {exc}")

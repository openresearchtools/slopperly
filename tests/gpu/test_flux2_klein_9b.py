...
def test_flux2_klein_9b_t2i(gpu_cert, plugin_loader, base_models, repo_root):
    ...
    workflow_pack = gpu_cert.require_file(
        ...
        "FLUX.2 Klein 9B Comfy T2I workflow API graph",
    )

    module = plugin_loader("image", "flux2_klein_9b")
    plugin = module.Flux2Klein9BPlugin()
    output_path = gpu_cert.artifact_path(LOGICAL_NAME, "flux2_klein_9b_t2i.png")
    ...
    try:
        ...
        gpu_cert.block(LOGICAL_NAME, f"FLUX.2 Klein 9B T2I Comfy plugin path runtime error: {exc}")
    except ArtifactValidationError as exc:
        gpu_cert.fail(LOGICAL_NAME, f"FLUX.2 Klein 9B T2I PNG validation failed: {exc}")
...
def test_flux2_klein_9b_edit(
    ...
    workflow_pack = gpu_cert.require_file(
        ...
        "FLUX.2 Klein 9B Comfy reference-edit workflow API graph",
    )
    ...
    module = plugin_loader("image", "flux2_klein_9b")
    plugin = module.Flux2Klein9BPlugin()
    output_path = gpu_cert.artifact_path(LOGICAL_NAME, "flux2_klein_9b_edit.png")
    module.solve_path = lambda filename: str(output_path)
    ...
    try:
        ...
        gpu_cert.block(LOGICAL_NAME, f"FLUX.2 Klein 9B edit Comfy plugin path runtime error: {exc}")
    except ArtifactValidationError as exc:
        gpu_cert.fail(LOGICAL_NAME, f"FLUX.2 Klein 9B edit PNG validation failed: {exc}")

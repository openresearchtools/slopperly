...
LOGICAL_NAME = "wan22_t2v_a14b_720p16_to24_gguf"
...
def test_wan22_t2v_a14b_q5_gguf_plugin_path(gpu_cert, plugin_loader, base_models, repo_root):
    ...
    workflow_pack = gpu_cert.require_file(
        ...
        "Wan2.2 A14B T2V Q5 GGUF Comfy workflow API graph",
    )
    model_files = {
        "high_noise_gguf": repo_root
        / ".slopperly/runtimes/ComfyUI/models/unet/HighNoise/Wan2.2-T2V-A14B-HighNoise-Q5_K_M.gguf",
        "low_noise_gguf": repo_root
        / ".slopperly/runtimes/ComfyUI/models/unet/LowNoise/Wan2.2-T2V-A14B-LowNoise-Q5_K_M.gguf",
        ...
        "high_lora": repo_root
        / ".slopperly/runtimes/ComfyUI/models/loras/wan2.2_t2v_lightx2v_4steps_lora_v1.1_high_noise.safetensors",
        "low_lora": repo_root
        / ".slopperly/runtimes/ComfyUI/models/loras/wan2.2_t2v_lightx2v_4steps_lora_v1.1_low_noise.safetensors",
    }
    for label, path in model_files.items():
        gpu_cert.require_file(LOGICAL_NAME, path, f"Wan2.2 A14B T2V {label} artifact")

    module = plugin_loader("video", "wan_t2v")
    plugin = module.WanT2VPlugin()
    output_path = gpu_cert.artifact_path(LOGICAL_NAME, "wan22_t2v_a14b_24fps.mp4")
    ...
    inputs = base_models.ModelInputs(
        prompt="a local Wan A14B text-to-video smoke test with gentle camera drift",
    ...
    try:
        ...
        gpu_cert.block(LOGICAL_NAME, f"Wan2.2 A14B T2V Comfy plugin path runtime error: {exc}")
    except ArtifactValidationError as exc:
        gpu_cert.fail(LOGICAL_NAME, f"Wan2.2 A14B T2V MP4 validation failed: {exc}")

...
LOGICAL_NAME = "ltx23_i2v_q5_gguf"
...
def test_ltx23_i2v_existing_workflow(gpu_cert, plugin_loader, base_models, repo_root):
    ...
    workflow_pack = gpu_cert.require_file(
        ...
        "LTX 2.3 Q5 GGUF Comfy I2V workflow API graph",
    )
    source = gpu_cert.require_file(
        LOGICAL_NAME,
        repo_root / "tests" / "fixtures" / "vsr_source.ppm",
        "LTX source image fixture",
    )

    module = plugin_loader("video", "ltx2")
    plugin = module.LTX2Plugin()
    output_path = gpu_cert.artifact_path(LOGICAL_NAME, "ltx23_i2v_q5.mp4")
    module.solve_path = lambda filename: str(output_path)

    inputs = base_models.ModelInputs(
        prompt="a second verified local LTX Q5 image-to-video shot with gentle camera motion",
    ...
    try:
        ...
        gpu_cert.block(LOGICAL_NAME, f"LTX 2.3 Q5 I2V Comfy plugin path runtime error: {exc}")
    except ArtifactValidationError as exc:
        gpu_cert.fail(LOGICAL_NAME, f"LTX 2.3 Q5 I2V MP4 validation failed: {exc}")

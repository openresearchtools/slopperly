                    ...
                    "prompt-1": {
                        "outputs": {
                            "17": {
                                "videos": [
                                    {
                                        "filename": "slopperly_wan22_i2v_a14b_native16_00001_.mp4",
                                        "subfolder": "video",
                                        "type": "output",
                                    }
                                ]
...
def _wan22_i2v_a14b_object_info() -> dict:
    workflow = json.loads(
        (
            ROOT / "slopperly/workflows/comfy/wan22_i2v_a14b_720p16_to24_gguf/workflow.api.json"
        ).read_text(encoding="utf-8")
    )
    return {node["class_type"]: {} for node in workflow.values()}
...
class ComfyWorkflowRunnerIntegrationTests(unittest.TestCase):
    ...
    def test_wan22_i2v_a14b_pack_patches_gguf_two_stage_graph(self):
        ComfyHandler.reset(_wan22_i2v_a14b_object_info())
        runner = ComfyWorkflowRunner(ComfyApiClient(self.base_url))
        with tempfile.TemporaryDirectory() as tmp:
            source = Path(tmp) / "source.png"
            source.write_bytes(b"local image fixture bytes")
            destination = Path(tmp) / "wan_a14b_native16.mp4"
            inputs = SimpleNamespace(
                prompt="local Wan A14B I2V motion",
                ...
                seed=220514,
                batch=1,
                wan_high_model="HighNoise/Wan2.2-I2V-A14B-HighNoise-Q5_K_M.gguf",
                wan_low_model="LowNoise/Wan2.2-I2V-A14B-LowNoise-Q5_K_M.gguf",
                ...
                wan_high_lora="wan2.2_i2v_lightx2v_4steps_lora_v1_high_noise.safetensors",
                wan_low_lora="wan2.2_i2v_lightx2v_4steps_lora_v1_low_noise.safetensors",
                ...
                wan_output_prefix="video/slopperly_wan22_i2v_a14b_native16",
            )

            with local_only_network():
                result = runner.run_pack(
                    ROOT / "slopperly/workflows/comfy/wan22_i2v_a14b_720p16_to24_gguf",
                    inputs,
                    SimpleNamespace(),
                    destination=str(destination),
                    timeout=2,
        ...
        self.assertEqual(prompt["8"]["inputs"]["expert_mode_allocations"], "cuda:0,1gb;cpu,*")
    ...
    def test_flux2_klein_4b_packs_patch_t2i_and_reference_edit_graphs(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            destination = Path(tmp) / "flux2_klein.png"
            runner = ComfyWorkflowRunner(ComfyApiClient(self.base_url))
            inputs = SimpleNamespace(
                prompt="local FLUX.2 Klein 4B workflow render",
                width=1024,
                height=1024,
                batch=1,
                seed=42004,
                steps=4,
                flux2_klein_guidance=1.0,
                flux2_klein_sampler="euler",
                flux2_klein_model="flux-2-klein-4b-fp8.safetensors",
            ...
            with local_only_network():
                result = runner.run_pack(
                    ROOT / "slopperly/workflows/comfy/flux2_klein_4b_t2i_edit",
                    inputs,
                    SimpleNamespace(),
                    destination=str(destination),
                    timeout=2,
        ...
        prompt = ComfyHandler.last_prompt
        self.assertEqual(ComfyHandler.upload_bodies, [])
        self.assertEqual(prompt["1"]["inputs"]["unet_name"], "flux-2-klein-4b-fp8.safetensors")
        self.assertEqual(prompt["2"]["inputs"]["clip_name"], "qwen_3_4b.safetensors")
        self.assertEqual(prompt["2"]["inputs"]["type"], "flux2")
        self.assertEqual(prompt["3"]["inputs"]["vae_name"], "flux2-vae.safetensors")
        self.assertEqual(prompt["4"]["inputs"]["text"], "local FLUX.2 Klein 4B workflow render")
        self.assertEqual(prompt["6"]["inputs"]["cfg"], 1.0)
        self.assertEqual(prompt["7"]["inputs"]["noise_seed"], 42004)
        self.assertEqual(prompt["8"]["inputs"]["sampler_name"], "euler")
        self.assertEqual(prompt["9"]["inputs"]["steps"], 4)
        self.assertEqual(prompt["9"]["inputs"]["width"], 1024)
        self.assertEqual(prompt["10"]["inputs"]["height"], 1024)

        ComfyHandler.reset(_flux2_klein_object_info("flux2_klein_4b_t2i_edit_img2img"))
        with tempfile.TemporaryDirectory() as tmp:
            source = Path(tmp) / "source.png"
            ref = Path(tmp) / "ref.png"
            destination = Path(tmp) / "flux2_klein_edit.png"
            source.write_bytes(b"local flux source image")
            ref.write_bytes(b"local flux reference image")
            runner = ComfyWorkflowRunner(ComfyApiClient(self.base_url))
            inputs.images = [str(source), str(ref), None]
            inputs.prompt = "local FLUX.2 Klein reference edit"
            inputs.seed = 42005

            with local_only_network():
                result = runner.run_pack(
                    ROOT / "slopperly/workflows/comfy/flux2_klein_4b_t2i_edit_img2img",
                    inputs,
                    SimpleNamespace(),
                    destination=str(destination),
                    timeout=2,
        ...
        self.assertEqual(prompt["1"]["inputs"]["image"], "uploaded_source.png")
        self.assertEqual(prompt["19"]["inputs"]["image"], "uploaded_source_2.png")
        self.assertEqual(prompt["6"]["inputs"]["text"], "local FLUX.2 Klein reference edit")
        self.assertEqual(prompt["11"]["inputs"]["positive"], ["27", 0])
        self.assertEqual(prompt["11"]["inputs"]["negative"], ["28", 0])
        self.assertEqual(prompt["12"]["inputs"]["noise_seed"], 42005)
        self.assertEqual(prompt["14"]["inputs"]["steps"], 4)
        self.assertEqual(prompt["22"]["inputs"]["latent"], ["21", 0])
        self.assertEqual(prompt["23"]["inputs"]["latent"], ["21", 0])
        self.assertNotIn("24", prompt)
        self.assertNotIn("25", prompt)
        self.assertNotIn("26", prompt)
        self.assertNotIn("latent", prompt["27"]["inputs"])
        self.assertNotIn("latent", prompt["28"]["inputs"])
        self.assertIn(b'filename="source.png"', ComfyHandler.upload_bodies[0])
        self.assertIn(b'filename="ref.png"', ComfyHandler.upload_bodies[1])

    def test_flux2_klein_9b_packs_patch_t2i_and_reference_edit_graphs(self):
        ComfyHandler.reset(_flux2_klein_object_info("flux2_klein_9b_t2i_edit"))
        with tempfile.TemporaryDirectory() as tmp:
            destination = Path(tmp) / "flux2_klein_9b.png"
            runner = ComfyWorkflowRunner(ComfyApiClient(self.base_url))
            inputs = SimpleNamespace(
                prompt="local FLUX.2 Klein 9B workflow render",
                width=1024,
                height=1024,
                batch=1,
                seed=42009,
                ...
                flux2_klein_guidance=1.0,
                flux2_klein_sampler="euler",
                flux2_klein_model="flux-2-klein-9b-fp8.safetensors",
                flux2_klein_text_encoder="qwen_3_8b_fp8mixed.safetensors",
                flux2_klein_clip_type="flux2",
                flux2_klein_vae="full_encoder_small_decoder.safetensors",
            )

            with local_only_network():
                result = runner.run_pack(
                    ROOT / "slopperly/workflows/comfy/flux2_klein_9b_t2i_edit",
                    inputs,
                    SimpleNamespace(),
                    destination=str(destination),
                    timeout=2,
        ...
        prompt = ComfyHandler.last_prompt
        self.assertEqual(ComfyHandler.upload_bodies, [])
        self.assertEqual(prompt["1"]["inputs"]["unet_name"], "flux-2-klein-9b-fp8.safetensors")
        self.assertEqual(prompt["2"]["inputs"]["clip_name"], "qwen_3_8b_fp8mixed.safetensors")
        self.assertEqual(prompt["2"]["inputs"]["type"], "flux2")
        self.assertEqual(prompt["3"]["inputs"]["vae_name"], "full_encoder_small_decoder.safetensors")
        self.assertEqual(prompt["4"]["inputs"]["text"], "local FLUX.2 Klein 9B workflow render")
        self.assertEqual(prompt["6"]["inputs"]["cfg"], 1.0)
        self.assertEqual(prompt["7"]["inputs"]["noise_seed"], 42009)
        self.assertEqual(prompt["8"]["inputs"]["sampler_name"], "euler")
        self.assertEqual(prompt["9"]["inputs"]["steps"], 4)
        self.assertEqual(prompt["9"]["inputs"]["width"], 1024)
        self.assertEqual(prompt["10"]["inputs"]["height"], 1024)

        ComfyHandler.reset(_flux2_klein_object_info("flux2_klein_9b_t2i_edit_img2img"))
        with tempfile.TemporaryDirectory() as tmp:
            source = Path(tmp) / "source.png"
            ref = Path(tmp) / "ref.png"
            destination = Path(tmp) / "flux2_klein_9b_edit.png"
            source.write_bytes(b"local flux source image")
            ref.write_bytes(b"local flux reference image")
            runner = ComfyWorkflowRunner(ComfyApiClient(self.base_url))
            inputs.images = [str(source), str(ref), None]
            inputs.prompt = "local FLUX.2 Klein 9B reference edit"
            inputs.seed = 42010

            with local_only_network():
                result = runner.run_pack(
                    ROOT / "slopperly/workflows/comfy/flux2_klein_9b_t2i_edit_img2img",
                    inputs,
                    SimpleNamespace(),
                    destination=str(destination),
                    timeout=2,
        ...
        self.assertEqual(prompt["1"]["inputs"]["image"], "uploaded_source.png")
        self.assertEqual(prompt["19"]["inputs"]["image"], "uploaded_source_2.png")
        self.assertEqual(prompt["3"]["inputs"]["unet_name"], "flux-2-klein-9b-fp8.safetensors")
        self.assertEqual(prompt["4"]["inputs"]["clip_name"], "qwen_3_8b_fp8mixed.safetensors")
        self.assertEqual(prompt["5"]["inputs"]["vae_name"], "full_encoder_small_decoder.safetensors")
        self.assertEqual(prompt["6"]["inputs"]["text"], "local FLUX.2 Klein 9B reference edit")
        self.assertEqual(prompt["11"]["inputs"]["positive"], ["27", 0])
        self.assertEqual(prompt["11"]["inputs"]["negative"], ["28", 0])
        self.assertEqual(prompt["12"]["inputs"]["noise_seed"], 42010)
        self.assertEqual(prompt["14"]["inputs"]["steps"], 4)
        self.assertEqual(prompt["22"]["inputs"]["latent"], ["21", 0])
        self.assertEqual(prompt["23"]["inputs"]["latent"], ["21", 0])
        self.assertNotIn("24", prompt)
    ...
    def test_flux_kontext_pack_uploads_reference_and_patches_edit_graph(self):
        ComfyHandler.reset(_flux_kontext_object_info())
        with tempfile.TemporaryDirectory() as tmp:
            source = Path(tmp) / "source.png"
            destination = Path(tmp) / "flux_kontext.png"
            ...
            inputs = SimpleNamespace(
                ...
                seed=6301,
                steps=28,
                flux_kontext_cfg=1.0,
                flux_kontext_sampler="euler",
            ...
            with local_only_network():
                result = runner.run_pack(
                    ROOT / "slopperly/workflows/comfy/flux_kontext_edit",
                    inputs,
                    SimpleNamespace(),
                    destination=str(destination),
                    timeout=2,

...
def get_enum_items(media_type: str, source: str = "LOCAL") -> list:
    ...
    ``source`` is retained for compatibility with older callers, but production
    Slopperly dropdowns are local-only. Hidden saved-project aliases remain in
    ``PLUGIN_REGISTRY`` and are deliberately omitted from enum items.

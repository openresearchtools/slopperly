...
Local ComfyUI workflow pack for the existing LTX 2.3 Q5 text-to-video path.

## Existing Addon Function

- Current plugin/function: `models_plugins/video/ltx2.py` when no image or video strip is selected.
...
- `width` -> node `14`, input `width`
- `height` -> node `14`, input `height`
...
`steps` and `guidance` are deliberately unmapped in this graph because it uses a fixed `ManualSigmas` schedule and `BasicGuider` without a CFG input.
...
```bash
pytest tests/gpu/test_ltx23_t2v_q5.py --device cuda
```

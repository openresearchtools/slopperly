# chatterbox_tts_vc_comfy

Local Chatterbox zero-shot reference TTS for `models_plugins/audio/chatterbox.py` when the existing speaker-reference picker supplies an audio file.
...
The pinned source exposes class key `FL_ChatterboxTTS` and accepts optional `audio_prompt` input for reference-voice conditioning.
...
- node `1`, output `0` -> `FL_ChatterboxTTS.audio_prompt`
...
`SaveAudio` writes one local audio artifact with prefix `slopperly_chatterbox_ref`. The plugin copies the first returned artifact to the existing result path and returns that path for VSE insertion.

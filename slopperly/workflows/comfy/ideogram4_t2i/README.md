...
Local ComfyUI text-to-image workflow pack for the existing `ideogram-ai/ideogram-4-nf4-diffusers` Ideogram 4 plugin.
...
These are ComfyUI core node classes from pinned ComfyUI commit `603d891eaf045d726d9c23276b4428daf2977624`. `Ideogram4Scheduler` is defined by Comfy core `comfy_extras/nodes_ideogram4.py`; `DualModelGuider`, `CFGOverride`, `KSamplerSelect`, `RandomNoise`, and `SamplerCustomAdvanced` are defined by Comfy core `comfy_extras/nodes_custom_sampler.py`.
...
The files are sourced from `Comfy-Org/Ideogram-4`. Hugging Face is used only as a local artifact source.
...
- Ideogram conditional model -> node `1`, input `unet_name`
- Ideogram unconditional model -> node `2`, input `unet_name`

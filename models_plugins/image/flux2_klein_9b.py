"""Text-to-image and reference image editing via local ComfyUI FLUX.2 Klein 9B."""
...
class Flux2Klein9BPlugin(ModelPlugin):
    ...
    DESCRIPTION  = "Text-to-image and reference editing via local ComfyUI FLUX.2 Klein 9B"
    MODEL_TYPE   = "image"
    ...
    def generate(self, pipe_obj, inputs: ModelInputs, scene, prefs):
        ...
        if inputs.mode in {"img2img", "inpaint"} and inputs.image is not None:
            ...
            stem = "flux2_klein_9b_edit"
            unet_node = "3"
            guider_node = "11"
            if inputs.mode == "inpaint":
                self._append_usage_note(
                    inputs,
                    "The committed FLUX.2 Klein 9B Comfy graph uses the official "
        ...
        if applied_loras:
            self._append_usage_note(
                inputs,
                f"FLUX.2 Klein 9B applied {applied_loras} selected LoRA(s) through "
    ...
    def _prepare_reference_images(self, inputs: ModelInputs, scene) -> None:
        ...
        if not images:
            raise ValueError(
                "FLUX.2 Klein 9B reference editing requires an image input. "

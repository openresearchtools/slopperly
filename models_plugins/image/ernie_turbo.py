"""Fast text-to-image via the local ERNIE-Image-Turbo Comfy workflow."""
...
class ErnieTurboPlugin(ModelPlugin):
    ...
    MODEL_TYPE = "image"
    DESCRIPTION = "Fast text-to-image via the local ERNIE-Image-Turbo Comfy workflow"
    ...
    def generate(self, pipe_obj, inputs: ModelInputs, scene, prefs):
        ...
        if inputs.neg_prompt:
            note = (
                "ERNIE-Image-Turbo's official Comfy template uses ConditioningZeroOut "
                "for the negative branch, so the negative prompt field is preserved "
                "in the UI but deliberately unmapped for this certified graph."
            )
            inputs.usage_note = f"{inputs.usage_note}\n{note}".strip()

        stem = clean_filename((inputs.prompt or "ernie_image_turbo")[:40]) or "ernie_image_turbo"
        destination = solve_path(f"ernie_image_turbo_{stem}.png")

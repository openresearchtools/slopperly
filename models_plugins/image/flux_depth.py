"""FLUX.1 Depth control generation via local ComfyUI."""
...
class FluxDepthPlugin(ModelPlugin):
    ...
    MODEL_TYPE   = "image"
    DESCRIPTION  = "Depth-guided generation through local ComfyUI FLUX.1 Depth"
    ...
    def generate(self, pipe_obj, inputs: ModelInputs, scene, prefs):
        ...
        self._append_usage_note(
            inputs,
            "The official FLUX.1 Depth LoRA Comfy graph uses InstructPixToPixConditioning "
        ...
        inputs.flux1_depth_t5 = "t5xxl_fp16.safetensors"
        inputs.flux1_depth_clip_type = "flux"
        inputs.flux1_depth_vae = "ae.safetensors"
        inputs.flux1_depth_negative_prompt = ""
        inputs.flux1_depth_flux_guidance = float(inputs.guidance or 3.5)
        inputs.flux1_depth_cfg = 2.0
        inputs.flux1_depth_sampler = "euler"
        inputs.flux1_depth_scheduler = "normal"
        inputs.flux1_depth_denoise = 1.0
        inputs.flux1_depth_preprocessor_ckpt = "depth_anything_v2_vitl.pth"
        inputs.flux1_depth_preprocessor_resolution = int(inputs.width or 1024)

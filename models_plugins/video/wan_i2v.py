"""Wan2.2 A14B image-to-video routed through owned local ComfyUI GGUF."""
...
except ImportError:  # GPU plugin harness only supplies helpers used by exercised paths.
    load_first_frame = None


WORKFLOW_ID = "wan22_i2v_a14b_720p16_to24_gguf"
...
def _safe_wan_a14b_dimensions(width: int, height: int) -> tuple[int, int]:
    """Map arbitrary UI sizes to the certified 720p Wan A14B family."""
...
def prepare_wan22_i2v_a14b_inputs(inputs: ModelInputs) -> None:
    ...
    inputs.wan_output_prefix = "video/slopperly_wan22_i2v_a14b_native16"


class WanI2VPlugin(ModelPlugin):
    MODEL_ID = "Wan-AI/Wan2.2-I2V-A14B"
    DISPLAY_NAME = "Video: Wan2.2 I2V A14B (Local Q5)"
    MODEL_TYPE = "video"
    DESCRIPTION = "Local Wan2.2 A14B image-to-video through ComfyUI-GGUF, 16fps native to 24fps final MP4."
    ...
    def generate(self, pipe_obj, inputs: ModelInputs, scene, prefs):
        ...
        if image is None and inputs.video_path:
            if load_first_frame is None:
                raise RuntimeError("Wan2.2 A14B I2V video-strip input requires load_first_frame.")
            image = load_first_frame(inputs.video_path)
        if image is None:
            raise RuntimeError("Wan2.2 A14B I2V requires an image or video input.")
        inputs.image = image

        prepare_wan22_i2v_a14b_inputs(inputs)
        if isinstance(pipe_obj, dict) and pipe_obj.get("enabled_loras"):
            _append_usage_note(
                inputs,
                "Wan2.2 A14B Q5 uses the committed high/low Lightx2v LoRAs; "
                "project LoRA adapters remain visible but need separate artifact certification.",
            )
        _append_usage_note(
            inputs,
            (
                f"Wan2.2 A14B Q5 generated {inputs.frames} native frames at "
        ...
        stem = clean_filename(str(inputs.seed) + "_" + (inputs.prompt[:30] or "wan22_i2v_a14b"))
        ...
        self.set_phase(inputs, "Finalizing Wan2.2 A14B output at 24fps")
...
def _finalize_native_16fps_to_24fps(native_path: Path, destination: Path) -> None:
    ...
    try:
        ...
        raise RuntimeError("ffmpeg is required to finalize Wan2.2 A14B I2V output") from exc
    except subprocess.CalledProcessError as exc:
        detail = (exc.stderr or exc.stdout or "").strip()
        raise RuntimeError(f"ffmpeg failed to finalize Wan2.2 A14B I2V output: {detail}") from exc
